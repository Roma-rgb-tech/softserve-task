locals {
  # Every chart is the metric its alert policy watches, aggregated the way the
  # policy aggregates it. Opening the dashboard after an alert then shows the
  # series that fired rather than a differently shaped view of it.
  ...
  charts = [
    ...
    merge(local.gauge, { title = "Disk used, %", metric = "agent.googleapis.com/disk/percent_used", extra = " AND metric.labels.state=\"used\"" }),
