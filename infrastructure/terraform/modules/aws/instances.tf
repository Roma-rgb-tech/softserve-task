...
resource "aws_instance" "workload" {
  ...
  vpc_security_group_ids = [for tag in each.value.network_tags : local.workload_groups[tag]]
  ...
  lifecycle {
    ...
    precondition {
      condition = (
        cidrhost(each.value.subnet_cidr, 0) ==
        cidrhost("${each.value.internal_ip}/${split("/", each.value.subnet_cidr)[1]}", 0)
      )
      error_message = "internal_ip ${each.value.internal_ip} is outside the subnet the ${each.value.role} VM is placed in (${each.value.subnet_cidr})."
    }

    precondition {
      condition     = !contains([for i in range(4) : cidrhost(each.value.subnet_cidr, i)], each.value.internal_ip)
      error_message = "internal_ip ${each.value.internal_ip} uses one of the first four addresses of its subnet, which AWS reserves."
    }
