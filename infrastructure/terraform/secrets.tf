...
resource "google_secret_manager_secret" "this" {
  for_each  = toset([for secret_id in local.all_secret_ids : secret_id if local.is_gcp])
...
resource "google_secret_manager_secret_iam_member" "workload_access" {
  for_each = { for pair in local.workload_secret_pairs : "${pair.vm_name}/${pair.secret_id}" => pair if local.is_gcp }
...
resource "google_secret_manager_secret_iam_member" "version_adder" {
  for_each = { for key, writer in local.secret_version_writers : key => writer if local.is_gcp }
...
resource "aws_secretsmanager_secret" "this" {
  for_each = toset([for secret_id in local.all_secret_ids : secret_id if local.is_aws])
...
data "aws_iam_policy_document" "version_adder" {
  for_each = toset([for secret_id in local.all_secret_ids : secret_id if local.is_aws && length(var.secret_version_manager_arns) > 0])
