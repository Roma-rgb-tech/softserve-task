...
module "gcp_vm" {
  ...
  subnet_id = each.value.role == "bastion" ? local.network.management_subnet_id : local.network.workload_subnet_id

  network_groups = [
    for tag in each.value.network_tags : local.network.workload_groups[tag]
  ]
