...
async def history_recent(limit: int = Query(20, ge=1, le=1000), offset: int = Query(0, ge=0)):
    """Paged history. Stays synchronous HTTP: RabbitMQ carries the write path
    only, and a queue is the wrong shape for a request/response read."""
...
async def history_count():
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        try:
            resp = await client.get(f"{HISTORY_BASE}/history/count")
        except Exception as e:
            raise HTTPException(status_code=502, detail=str(e))
    return resp.json()
